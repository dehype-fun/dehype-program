...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        if (!existingPendingUser) {
            ...
            return res.status(201).send(newPendingUser);
        } else {
            return res.status(200).send(existingPendingUser);
        }
    } catch (error) {
        console.error(`Error processing user registration: ${(error as Error).message}`);
...
router.post('/confirm', async (req, res) => {
    ...
    if (!body.isLedger) {
        ...
        if (feePayer != body.wallet) {
            console.log("feePayer", feePayer, body.wallet)
